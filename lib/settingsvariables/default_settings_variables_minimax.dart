// Default location: lib/settingsvariables/default_settings_variables.dart
// default settings values
// if you want to use minimax change file name as default_settings_variables, dont forget to change other one to default_settings_variables_openrouter

import '../models/bot.dart';
import 'package:uuid/uuid.dart';

// Varsayılan bot listesi (bot_provider.dart ile aynı)
final List<Bot> defaultBots = [
  Bot(
    id: const Uuid().v4(),
    name: 'Minimax M2.7',
    model: 'minimax-m2.7',
    iconName: 'edit',
  ),
  Bot(
    id: const Uuid().v4(),
    name: 'Minimax M3.0',
    model: 'minimax-m3.0',
    iconName: 'smart_toy',
  ),
  Bot(
    id: const Uuid().v4(),
    name: 'Minimax M2.0',
    model: 'minimax-m2.0',
    iconName: 'smart_toy',
  ),
];

double defaultTemperature = 0.7;
int defaultMaxTokens = 2048;
String defaultSystemPrompt =
    """You are a helpful assistant that formats responses clearly. 

When writing mathematical expressions or equations, you MUST use LaTeX syntax in your responses.

For inline equations, use: \$\$equation here\$\$
For block equations, use: [equation here]

When writing code, always use code blocks with language specification:
```python
print("Hello World")
```

Format all your responses with proper markdown.""";

const String baseUrl = 'https://www.minimax.io';
const String defaultControlModel =
    'minimax-m2.0'; //this is for the test connections and for the session title
const String nousablebot =
    'minimax-m2.0'; // If no available models or selected model not in list, use default
String apikey = ''; // dont add your apikey here, leave it empty
